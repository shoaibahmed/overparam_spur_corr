...
class VariableWidthResNet(nn.Module):
    ...
    def __init__(self, block, layers, width, num_classes=1000, zero_init_residual=False,
        ...
        print("Using a fixed projection layer...")
        self.proj_dim = 128
        self.projector = nn.Sequential(nn.Linear(self.feature_size, self.proj_dim), nn.BatchNorm1d(self.proj_dim), nn.ReLU(inplace=True))
        self.fc = nn.Linear(self.proj_dim, num_classes)

        self.feature_size_conv = self.feature_size
        self.feature_size = self.proj_dim
